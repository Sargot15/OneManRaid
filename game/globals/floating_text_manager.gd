...
func show_damage_text(amount : int, world_position : Vector2) -> void:
	show_text(amount, world_position, DAMAGE_COLOR)

func show_damage_shield_text(amount : int, world_position : Vector2) -> void:
	show_text(amount, world_position, DAMAGE_SHIELD_COLOR)

func show_heal_text(amount : int, world_position : Vector2) -> void:
	show_text(amount, world_position, HEAL_COLOR)

func show_text(amount : int, world_position : Vector2, color : Color) -> void:
	var text_instance = FloatingTextScene.instantiate()
	text_instance.text = " " + str(amount) + " " # Added spaces so the outline of the text does not get cut
